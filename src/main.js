...
function foot(dt){
 ...
 player.position.x=THREE.MathUtils.clamp(player.position.x,-680,680);player.position.z=THREE.MathUtils.clamp(player.position.z,-680,680);
...
canvas.addEventListener('click',()=>{if(onFoot&&!paused&&document.pointerLockElement!==canvas)canvas.requestPointerLock()});addEventListener('mousemove',e=>{if(onFoot&&document.pointerLockElement===canvas){playerYaw-=e.movementX*.0028;playerPitch=THREE.MathUtils.clamp(playerPitch-e.movementY*.002,-0.35,.65)}});addEventListener('keydown',e=>{keys[e.key]=true;if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight',' '].includes(e.key))e.preventDefault();const k=e.key.toLowerCase();if(k==='r')reset();if(k==='c')third=!third;if(k==='e')toggleVehicle();if(e.key==='Escape')(paused?close:open)()});addEventListener('keyup',e=>keys[e.key]=false);addEventListener('resize',()=>{camera.aspect=innerWidth/innerHeight;camera.updateProjectionMatrix();renderer.setSize(innerWidth,innerHeight)});setInterval(()=>exitCooldown=Math.max(0,exitCooldown-.1),100);setup();requestAnimationFrame(loop);